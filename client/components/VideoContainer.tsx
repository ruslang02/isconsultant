...
const VideoContainer: React.FC<{
    ...
    useEffect(() => {
        ...
        function onMessage(message: any) {
            ...
            if (data.videoroom == "event") {
                const participants = data.publishers;
                if (participants) {
                ...
                if (left) {
                    ...
                    setState(e => {
                        const i = e.findIndex(v => v.id == left);
                        if (i == -1) return e;
                        const n = [...e];
                        n.splice(i, 1);
                        return n;
                ...
                const leaving: string = data.leaving;
        ...
        !publisherHandle.current.getUserMedia
            ...
            function go(vi: boolean, au: boolean) {
                ...
                publisherHandle.current
                    ...
                    .then(function (stream: any) {
                        const pc = publisherHandle.current.createPeerConnection();
        ...
        function onVideoroomJoin(response: any) {
            ...
            const participants = response.getPluginData().publishers;
            const newUsers: { id; name }[] = [];
            participants.forEach((element: any) => {
    ...
    function clickScreen(enabled: boolean) {
        function onScreenJoin() {
            ...
            navigator.mediaDevices.getDisplayMedia()
                .then(function (stream: any) {
                    const pc = screenHandle.current.createPeerConnection();
                    userStream.current = stream;
                    stream.getTracks().forEach(function (track: any) {
    ...
    return (
        <section className={styles.Video_container}
            ...
            <Actions audioAvailable={audioAvailable}
